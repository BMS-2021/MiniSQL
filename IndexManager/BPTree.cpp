...
void BPTree::insert(uint32_t idx_pos, ELEMENTTYPE rec_id, const sql_value &target) {
    /*
    ...
    */
    ...
    sql_value tar = target;  // target value (actually to find, will change when split up)
    BPTreeNode *child = nullptr;

    while (true) {                              // from botton to top
        auto tmp = cur->binary_search(idx_pos, tar);  // find pos to insert
        if (tmp >= cur->key.size()) {
        ...
        tar = find_idx_val(idx_pos, new_key);
        child = sibling;
...
ELEMENTTYPE BPTree::search(uint32_t idx_pos, const sql_value& target) const {
    BPTreeNode *cur = root;
    if (cur == nullptr) {
    ...
    while (cur->ch.at(0) != nullptr) {
        auto i = 0U;
        for (; i < cur->key.size(); ++i)
            if (find_idx_val(idx_pos, cur->key.at(i)) > target)
                break;
        cur = cur->ch[i];
    }
    auto tmp = cur->binary_search(idx_pos, target);
    if (tmp == 0 || find_idx_val(idx_pos, cur->key.at(tmp - 1)) != target) {
        throw sql_exception(300, "index manager", "element not found");
    }
    return cur->key.at(tmp - 1);
}

void BPTree::remove(uint32_t idx_pos, const sql_value&target) {
    /*
  ...
  `		2. size not enough -> rotate
    ...
    */
    ...
    while (true) {
        ...
        auto get_wh = cur->fa->binary_search(idx_pos, target);
...
ELEMENTTYPE BPTree::searchNext(uint32_t idx_pos, const sql_value &target) const {
    BPTreeNode *cur = root;
    if (cur == nullptr) {
        throw sql_exception(300, "index manager", "element not found");
    }
